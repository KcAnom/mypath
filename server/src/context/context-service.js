...
export class ContextService {
  ...
  mentions(projectId, query = '') { const q = String(query).toLowerCase(); const state = this.database.loadState(); const out = []; const add = (type, id, label, detail = '', metadata = {}) => { if (!q || `${label} ${detail}`.toLowerCase().includes(q)) out.push({ type, id, label, detail, ...metadata }); }; for (const component of state.components.filter((item) => item.projectId === projectId && !item.deletedAt)) { add('component', component.id, component.name || component.generatedName, component.selectedRevisionId || '', { componentId: component.id, revisionId: component.selectedRevisionId || '' }); for (const revision of state.revisions.filter((item) => item.componentId === component.id)) add('revision', revision.id, `${component.name || component.generatedName} revision`, revision.id, { componentId: component.id, revisionId: revision.id }); } for (const asset of this.db.prepare('SELECT * FROM asset_ingestions WHERE project_id=? AND tombstoned_at IS NULL').all(projectId)) add('asset', asset.id, asset.original_name, asset.kind, { assetId: asset.id }); for (const design of this.designSystems?.list() || state.designSystems) add('design-system', design.id, design.name || design.id, design.currentVersion?.id || '', { designSystemId: design.id, versionId: design.currentVersion?.id || '' }); for (const library of this.libraries?.list() || state.libraries) add('library', library.id, library.name || library.id, '', { libraryId: library.id }); for (const skill of this.skills?.list() || state.skills) add('skill', skill.id, skill.name || skill.id, '', { skillId: skill.id }); return out.slice(0, 50); }
}
