...
type Mention = { type: string; id: string; label: string; detail?: string; componentId?: string; revisionId?: string; assetId?: string; designSystemId?: string; versionId?: string; libraryId?: string; skillId?: string };
type AttachedAsset = { id: string; name: string; kind: string };
...
export function ProjectCanvas({ projectId, project, components, navigate, onGenerate }: any) {
  ...
  const [selected, setSelected] = useState<any[]>([]); const [status, setStatus] = useState<Status>({ kind: 'info', text: 'Loading canvas…' });
  const [conflict, setConflict] = useState<CanvasRecord | null>(null);
  const [mentionText, setMentionText] = useState(''); const [mentions, setMentions] = useState<Mention[]>([]); const [selectedMentions, setSelectedMentions] = useState<Mention[]>([]); const [attachedAssets, setAttachedAssets] = useState<AttachedAsset[]>([]);
  ...
  const refreshOperational = useCallback(async () => { try { setOperational(await api.get<any>(`/api/v1/projects/${projectId}/operational-context`)); } catch (reason) { setStatus({ kind: 'error', text: `Operational context failed: ${errorText(reason)}` }); } }, [projectId]);
  useEffect(() => { void refreshOperational(); }, [refreshOperational]);
  const runAction = async (name: string, task: () => Promise<void>) => { if (actionBusy) return; setActionBusy(name); try { await task(); } catch (reason) { setStatus({ kind: 'error', text: `${name} failed: ${errorText(reason)}` }); } finally { setActionBusy(''); } };
  ...
  const applyRemoteCanvas = useCallback((instance: Editor, canvas: CanvasRecord) => {
    ...
    try {
      ...
      version.current = canvas.version; setConflict(null); setStatus({ kind: 'success', text: `Reloaded canvas v${canvas.version}` });
  ...
  const persist = useCallback(async (instance: Editor) => {
    ...
    try { const saved = await api.put<CanvasRecord>(`/api/v1/projects/${projectId}/canvas`, { version: version.current, snapshot, camera }); version.current = saved.version; setConflict(null); setStatus({ kind: 'success', text: `Saved v${saved.version}` }); }
    catch (error: any) {
      if (error.status !== 409 || !error.details?.current) { setStatus({ kind: 'error', text: `Save failed: ${errorText(error)}` }); return; }
      ...
      setStatus({ kind: 'error', text: 'Conflict: canvas changed elsewhere; local edits were not uploaded' });
    }
  }, [projectId]);
  ...
  const mounted = useCallback((instance: Editor) => {
    ...
    void (async () => {
      ...
      applyRemoteCanvas(instance, canvas); setStatus({ kind: 'success', text: `Canvas v${canvas.version}` }); loading = false;
    ...
    })().catch((error) => setStatus({ kind: 'error', text: `Canvas failed: ${errorText(error)}` }));
  }, [applyRemoteCanvas, persist, projectId]);

  const searchMentions = async (value: string) => { setMentionText(value); const query = value.includes('@') ? value.slice(value.lastIndexOf('@') + 1) : value; if (!value.includes('@')) { setMentions([]); return; } try { setMentions(await api.get(`/api/v1/projects/${projectId}/mentions?q=${encodeURIComponent(query)}`)); } catch (reason) { setMentions([]); setStatus({ kind: 'error', text: `Context search failed: ${errorText(reason)}` }); } };
  const snapshotContext = async (options: { prompt?: string; skillIds?: string[]; allowDescriptionActivation?: boolean } = {}) => {
    const shapeIds = selected.map((shape) => shape.id);
    const componentRefs = selected.filter((shape) => shape.type === 'design-frame' && shape.props.revisionId).map((shape) => ({ componentId: shape.props.componentId, revisionId: shape.props.revisionId }));
    for (const mention of selectedMentions) if (mention.componentId && mention.revisionId) componentRefs.push({ componentId: mention.componentId, revisionId: mention.revisionId });
    const uniqueComponents = [...new Map(componentRefs.map((ref) => [`${ref.componentId}:${ref.revisionId}`, ref])).values()];
    const assetIds = [...new Set([...attachedAssets.map((asset) => asset.id), ...selectedMentions.map((mention) => mention.assetId || '').filter(Boolean)])];
    const skillIds = [...new Set([...(options.skillIds || selectedSkillIds), ...selectedMentions.map((mention) => mention.skillId || '').filter(Boolean)])];
    const designSystems = selectedMentions.filter((mention) => mention.versionId).map((mention) => ({ designSystemId: mention.designSystemId, versionId: mention.versionId }));
    const libraries = selectedMentions.filter((mention) => mention.libraryId).map((mention) => ({ libraryId: mention.libraryId }));
    const context = await api.post<any>(`/api/v1/projects/${projectId}/context-snapshots`, { shapeIds, components: uniqueComponents, assets: assetIds.map((assetId) => ({ assetId })), designSystems, libraries, skills: skillIds.map((skillId) => ({ skillId })), prompt: options.prompt || '', allowDescriptionActivation: options.allowDescriptionActivation === true }); setContextSnapshot(context); setStatus({ kind: 'success', text: 'Context frozen for this canvas version.' }); return context;
  };
  const upload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const asset = await api.upload<any>(`/api/v1/projects/${projectId}/assets`, file, kind); setAttachedAssets((current) => current.some((item) => item.id === asset.id) ? current : [...current, { id: asset.id, name: asset.name, kind: asset.kind }]); setStatus({ kind: 'success', text: `Stored and attached ${asset.name}` });
      if (kind === 'font') { const registered = await api.post<any>('/api/v1/fonts', { assetId: asset.id, family: file.name.replace(/\.(?:woff2?|ttf|otf)$/i, '') }); await api.put(`/api/v1/projects/${projectId}/fonts/${encodeURIComponent(registered.id)}`, { active: true }); await refreshOperational(); setStatus({ kind: 'success', text: `Stored and activated local font ${registered.family}` }); }
    ...
    } catch (reason) { setStatus({ kind: 'error', text: `Upload failed: ${errorText(reason)}` }); } finally { setActionBusy(''); event.target.value = ''; }
  };
  const generate = async () => { if (!promptText.trim() || building) return; setBuilding(true); try { await onGenerate(promptText); setPromptText(''); setStatus({ kind: 'success', text: 'Generated component added to project.' }); } catch (reason) { setStatus({ kind: 'error', text: `Generation failed: ${errorText(reason)}` }); } finally { setBuilding(false); } };
  const reuseLibraryItem = async (event: DragEvent, member?: any) => {
    event.preventDefault(); let value = member; try { value ||= JSON.parse(event.dataTransfer.getData('application/x-mypath-library') || 'null'); } catch { setStatus({ kind: 'error', text: 'Library drop failed: the dragged item was not readable.' }); return; } if (!value) return;
    await runAction('Library reuse', async () => { const point = !member && editor.current && Number.isFinite(event.clientX) ? editor.current.screenToPage({ x: event.clientX, y: event.clientY }) : { x: 120, y: 120 }; await api.post(`/api/v1/projects/${projectId}/canvas/library-items`, { ...value, x: point.x, y: point.y }); const canvas = await api.get<CanvasRecord>(`/api/v1/projects/${projectId}/canvas`); if (editor.current) applyRemoteCanvas(editor.current, canvas); setStatus({ kind: 'success', text: 'Reused library component on the canvas.' }); });
  };
  ...
  const selectMention = (mention: Mention) => { setSelectedMentions((current) => current.some((item) => item.type === mention.type && item.id === mention.id) ? current : [...current, mention]); if (mention.skillId) setSelectedSkillIds((current) => current.includes(mention.skillId!) ? current : [...current, mention.skillId!]); setMentionText(''); setMentions([]); };
  const contextLabels = [...selected.map((shape) => `${shape.type} · ${shape.props?.title || shape.id.slice(6, 14)}`), ...selectedMentions.map((mention) => `@${mention.label}`), ...attachedAssets.map((asset) => `${asset.kind} · ${asset.name}`)];
  ...
  return <div className="tldraw-workspace">
    ...
    <aside className="canvas-context-panel"><header><strong>{project.name}</strong>{status && <span className={`status-message inline ${status.kind}`} role={status.kind === 'error' ? 'alert' : 'status'}>{status.text}</span>}</header>
      <div className="canvas-panel-body">
        ...
        <div className="context-pills">{selected.map((shape) => <span className="context-pill" key={shape.id}>{shape.type} · {shape.props?.title || shape.id.slice(6, 14)}</span>)}{selectedMentions.map((mention) => <button type="button" className="context-pill selected-skill" key={`${mention.type}:${mention.id}`} onClick={() => setSelectedMentions((current) => current.filter((item) => item !== mention))}>@{mention.label} ×</button>)}{attachedAssets.map((asset) => <button type="button" className="context-pill selected-skill" key={asset.id} onClick={() => setAttachedAssets((current) => current.filter((item) => item.id !== asset.id))}>{asset.kind} · {asset.name} ×</button>)}{!contextLabels.length && <small>No selection: context will still pin this exact canvas version.</small>}</div>
        ...
        <div className="mention-box"><input value={mentionText} onChange={(e) => void searchMentions(e.target.value)} placeholder="Attach context with @…"/>{mentions.map((mention) => <button key={`${mention.type}:${mention.id}`} onClick={() => selectMention(mention)}>{mention.label}<small>{mention.type} {mention.detail}</small></button>)}</div>
        <ProjectChatPanel projectId={projectId} contextSnapshot={contextSnapshot} contextLabels={contextLabels} skills={operational.skills || []} selectedSkillIds={selectedSkillIds} onSkillToggle={toggleSkill} prepareContext={snapshotContext}/>
