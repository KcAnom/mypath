...
export function PreviewFrame({ componentId, revisionId, diagnostics = [], title = 'Runnable preview', selectableSourceIds = [], onSelect }: Props) {
  ...
  useEffect(() => {
    const receive = (event: MessageEvent) => {
      ...
      if (data.type === 'ready') setReady(true);
        ...
        void api.post(`/api/v1/revisions/${revisionId}/screenshots`, { dataUrl: data.dataUrl, width: data.width, height: data.height }).then(() => setScreenshotStatus('Screenshot stored immutably')).catch((reason) => setScreenshotStatus(`Screenshot failed: ${reason?.message || reason}`));
      } else if (data.type === 'screenshot-error' && data.id === captureId.current) setScreenshotStatus(`Screenshot failed: ${String(data.message || 'capture error')}`);
    };
  ...
  return <section className="preview-panel" aria-label={title}>
    <header className="preview-toolbar">
      ...
      <div className="viewport-controls" aria-label="Preview viewport">
        {(Object.keys(viewports) as Array<keyof typeof viewports>).map((name) =>
          <button type="button" className={viewport === name ? 'primary' : 'ghost'} onClick={() => setViewport(name)} key={name}>{name}</button>)}
        {revisionId && <button type="button" className="screenshot-control" disabled={!ready} onClick={saveScreenshot}>Capture screenshot</button>}
      </div>
    </header>
    ...
    {screenshotStatus && <div className="build-ok screenshot-status">{screenshotStatus}</div>}
