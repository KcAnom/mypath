...
export function GlobalErrorBanner() {
  ...
  useEffect(() => {
    ...
    const runtime = (event: ErrorEvent) => setError(`The page encountered an error: ${event.message || message(event.error)}`);
  ...
  return error ? <div className="global-error-banner" role="alert"><span>{error}</span><button onClick={() => setError('')} aria-label="Dismiss unexpected error">Dismiss</button></div> : null;
}
